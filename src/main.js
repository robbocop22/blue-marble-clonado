...
inject(() => {
  ...
  window.addEventListener("message", (event) => {
    ...
    console.log(
      `Blob fetch took %c${String(Math.floor(elapsed / 60000)).padStart(2, "0")}:${String(
        Math.floor(elapsed / 1000) % 60
      ).padStart(2, "0")}.${String(elapsed % 1000).padStart(3, "0")}%c MM:SS.mmm`,
      consoleStyle,
      ""
    );
    ...
    if (contentType.includes("application/json")) {
      ...
      return new Promise((resolve) => {
        ...
        console.log(
          ...
          ).padStart(2, "0")}:${String(Math.floor(elapsed / 1000) % 60).padStart(2, "0")}.${String(
            elapsed % 1000
          ).padStart(3, "0")} MM:SS.mmm`
        );
  ...
  const observer = new MutationObserver((mutations, observer) => {
    ...
    if (!move) {
      ...
      move.onclick = function () {
        ...
        roundedBox.parentNode.className = roundedBox.parentNode.className.replace(
  ...
  overlayMain
    ...
    .addButton({ id: "bm-button-create", textContent: "Create" }, (instance, button) => {
      button.onclick = () => {
        ...
        templateManager.createTemplate(input.files[0], input.files[0]?.name.replace(/\.[^/.]+$/, ""), [
    ...
    .addButton(
      { id: "bm-button-convert", className: "bm-help", innerHTML: "🎨", title: "Template Color Converter" },
      (instance, button) => {
...
function buildOverlayTabTemplate() {
  overlayTabTemplate
    ...
    .buildElement()
    .addButton({ className: "bm-button-minimize", textContent: "↑" }, (instance, button) => {
      button.onclick = () => {
